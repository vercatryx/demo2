...
stops_for_date as (
  select
    ...
    coalesce(cs.deliveries_allowed, true) as status_allows_deliveries
  from public.stops s
  left join public.clients c on c.id = s.client_id
  left join public.client_statuses cs on cs.id = c.status_id
...
stops_filtered as (
  ...
  where client_paused is distinct from true
    ...
    and status_allows_deliveries is distinct from false
