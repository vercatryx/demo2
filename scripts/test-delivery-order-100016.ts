/**
 * Test script to process orders through the delivery process
 * with a test URL (bypassing R2 upload)
 * 
 * This script uses the same logic as app/delivery/actions.ts
 * 
 * Usage: npx tsx scripts/test-delivery-order-100016.ts [order_number]
 * Example: npx tsx scripts/test-delivery-order-100016.ts 100016
 */
...
// Get order number from command line argument or use default
const ORDER_NUMBER = process.argv[2] ? parseInt(process.argv[2], 10) : 100016;
